...
public:
    ...
    {
        if (!m_queue.empty()
            && m_queue.front().bytesWritten != m_queue.front().adu.size())
        {
            qCDebug(QT_MODBUS) << "(RTU client) Modbus recive data before write data success";
            return;
        }
